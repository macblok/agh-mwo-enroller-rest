...
public class ParticipantRestController {
	...
	@Autowired
	ParticipantService participantService; // spring zainicjuje obiekt typu ParticipantService dlatego nie ma new

	@RequestMapping(value = "", method = RequestMethod.GET)
	// GET localhost:8080/participants
	public ResponseEntity<?> getParticipants() { //
		Collection<Participant> participants = participantService.getAll();
		return new ResponseEntity<Collection<Participant>>(participants, HttpStatus.OK); // pod spodem jest Jackson,
																							// ktory konwertuje klase
																							// java do json
	}
	...
	public ResponseEntity<?> deleteParticipant(@PathVariable("id") String login) {
		Participant participant = participantService.findByLogin(login);
		if (participant == null) {
			return new ResponseEntity("Unable to delete. Participant with given login doesn't exist.", HttpStatus.NOT_FOUND);
		}
